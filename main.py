...
class AddressBook(UserDict):
    ...
    def add_record(self, record):
        ...
        return f"Contact with name: '{record.name.value}'" + \
               (f", and phone number: '{record.phones[-1]}'" if record.is_entered_phone is not False else "") + \
        ...
        for name, contact in self.data.items():
            if name_or_phone == name:
                ...
                if contact.birthday.value:
                    ...
                    birthday_show = f". Birthday in {user_birthday}, {days_to_birthday} days left"
    ...
    def days_to_birthday(self):
        if self.birthday.value:
            ...
            if date_user.month <= date_now.month and date_user.day < date_now.day:
                next_birthday = datetime(year=date_now.year + 1,
                                         month=date_user.month,
                                         day=date_user.day).date()
            else:
                next_birthday = datetime(year=date_now.year,
                                         month=date_user.month,
                                         day=date_user.day).date()
...
def handler(key, entered_str):
    ...
    if entered_str:
        ...
        elif re.match(r"[a-zA-Z]\w+", entered_str) is None:
            raise UserNameError()
